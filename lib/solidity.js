...
// ----------------------------------------------------------------
//  This plugin uses the following JS script
//   - https://github.com/alincode/codemirror-solidity/index.js
// ----------------------------------------------------------------
const CodeMirror = require("codemirror");

CodeMirror.defineMode("solidity", function (config) {
  let indentUnit = config.indentUnit; // let functionKeyword = 'function'
  ...
  let natSpecTags = ["title", "author", "notice", "dev", "param", "return"]; // let functionStructureStage = [{
  ...
  let atoms = {
    ...
    new: true,
    true: true,
    false: true //  "iota": true, "nil": true, "append": true,
  ...
  function tokenBase(stream, state) {
    ...
    if (keywords.propertyIsEnumerable(cur)) {
      ...
      if (keywordsSpecial.propertyIsEnumerable(cur)) state.lastToken = cur; //if (cur == 'function' && state.para == 'parameterMode')
    ...
    if (keywordsControlStructures.propertyIsEnumerable(cur) || keywordsTypeInformation.propertyIsEnumerable(cur) || keywordsV0505NewReserve.propertyIsEnumerable(cur)) return "keyword";
    ...
    if (keywordsMoreBlockAndTransactionProperties.propertyIsEnumerable(cur) || keywordsBlockAndTransactionProperties[cur] && keywordsBlockAndTransactionProperties[cur].some(function (item) {
      return stream.match(`.${item}`);
    })) return "variable-2";
    if (keywordsAbiEncodeDecodeFunctions[cur] && keywordsAbiEncodeDecodeFunctions[cur].some(function (item) {
      return stream.match(`.${item}`);
    })) return "abi";
